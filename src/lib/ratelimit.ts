 ...
 * external services.
 *
 * The AI roadmap generation is throttled per user per minute (a burst guard so
 * the free NIM tier is not hammered) but has NO daily cap.
 */
...
function devIncrement(key: string, limit: number): void {
  ...
  if (bucket.count >= limit) {
    incrementMetric("ratelimit.rejected");
